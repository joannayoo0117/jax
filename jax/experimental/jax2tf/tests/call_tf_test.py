...
class CallTfTest(tf_test_util.JaxToTfTestCase):
  ...
  def test_with_var_read_other_device(self, with_jit=True):
    ...
    x = np.array([2., 5.,], dtype=np.float32)
    res = _maybe_jit(with_jit, jax2tf.call_tf(fun_tf, tf_device_name=tf_devices[1].name))(x)
    self.assertAllClose(x * outer_var_array + 1., res, check_dtypes=False)

    # If we are under jit and do not use a `tf_device_name` we get an error
    # because the tf.Variable is in another device.
    if with_jit:
      with self.assertRaisesRegex(ValueError,
                                  r"Error compiling TensorFlow function"):
        _maybe_jit(with_jit, jax2tf.call_tf(fun_tf))(x)
